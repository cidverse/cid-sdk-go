		...
		if tag != "" {
			if field.Type.Kind() == reflect.Int {
				...
				if isSet {
					...
					reflect.ValueOf(data).Elem().Field(i).SetInt(int64(valAsInt))
				}
			} else if field.Type.Kind() == reflect.String {
				val, isSet := os.LookupEnv(tag)
				if isSet {
					reflect.ValueOf(data).Elem().Field(i).SetString(val)
				}
			} else if field.Type.Kind() == reflect.Bool {
				val, isSet := os.LookupEnv(tag)
				if isSet {
					valAsBool, _ := strconv.ParseBool(val)
					reflect.ValueOf(data).Elem().Field(i).SetBool(valAsBool)
				}
			}
		}
	}
